...
impl<'a> Image<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, String> {
        ...
        if read_u32(bytes, e_lfanew)? != PE_SIG {
        ...
        let subsystem = read_u16(bytes, oh_off + 68)?;
        let dll_characteristics = read_u16(bytes, oh_off + 70)?;

        let (image_base, num_dirs_off, data_dir_off) = if is_64 {
            (read_u64(bytes, oh_off + 24)?, oh_off + 108, oh_off + 112)
        } else {
            (read_u32(bytes, oh_off + 28)? as u64, oh_off + 92, oh_off + 96)
        };
        let num_dirs = read_u32(bytes, num_dirs_off)? as usize;
        ...
        for i in 0..number_of_sections as usize {
            ...
            sections.push(Section {
                ...
                raw_address: read_u32(bytes, off + 20)?,
        ...
        Ok(Image {
            ...
            file_size,
            is_64,
            file_header: FileHeader { machine, number_of_sections, characteristics },
            optional_header: OptionalHeader {
                image_base, address_of_entry_point, size_of_image, size_of_headers,
                subsystem, dll_characteristics, data_directories,
            },
            sections,
        })
    }
    ...
    pub fn imports(&self) -> Result<Vec<Import>, String> {
        let dir = self.optional_header.data_directories.get(1).copied()
            .unwrap_or(DataDirectory { virtual_address: 0, size: 0 });
        if dir.virtual_address == 0 { return Ok(Vec::new()); }
        let start = self.rva_to_offset(dir.virtual_address)
            .ok_or("import directory RVA bad")?;
        let mut out = Vec::new();
        let mut pos = start;
        loop {
            let oft = read_u32(self.bytes, pos)?;
            let name_rva = read_u32(self.bytes, pos + 12)?;
            let first_thunk = read_u32(self.bytes, pos + 16)?;
            if name_rva == 0 && first_thunk == 0 { break; }
            pos += 20;
            let name_off = self.rva_to_offset(name_rva).ok_or("dll name RVA")?;
            let dll = read_cstring(self.bytes, name_off);
            let lookup_rva = if oft != 0 { oft } else { first_thunk };
            let mut lookup_off = self.rva_to_offset(lookup_rva).ok_or("lookup RVA")?;
            let stride = if self.is_64 { 8 } else { 4 };
            let ord_flag: u64 = if self.is_64 { 0x8000_0000_0000_0000 } else { 0x8000_0000 };
            let mut functions = Vec::new();
            loop {
                let entry: u64 = if self.is_64 {
                    read_u64(self.bytes, lookup_off)?
                } else {
                    read_u32(self.bytes, lookup_off)? as u64
                };
                if entry == 0 { break; }
                if entry & ord_flag != 0 {
                    functions.push(ImportFn::Ordinal((entry & 0xffff) as u16));
                } else {
                    let n_off = self.rva_to_offset(entry as u32).ok_or("name entry RVA")?;
                    functions.push(ImportFn::Named(read_cstring(self.bytes, n_off + 2)));
                }
                lookup_off += stride;
            }
    ...
    pub fn exports(&self) -> Result<Vec<Export>, String> {
        let dir = self.optional_header.data_directories.first().copied()
            .unwrap_or(DataDirectory { virtual_address: 0, size: 0 });
        if dir.virtual_address == 0 { return Ok(Vec::new()); }
        let off = self.rva_to_offset(dir.virtual_address).ok_or("export RVA bad")?;
        let base_ordinal = read_u32(self.bytes, off + 16)?;
        let n_funcs = read_u32(self.bytes, off + 20)? as usize;
        let n_names = read_u32(self.bytes, off + 24)? as usize;
        let funcs_rva = read_u32(self.bytes, off + 28)?;
        let names_rva = read_u32(self.bytes, off + 32)?;
        let ords_rva = read_u32(self.bytes, off + 36)?;
        let funcs_off = self.rva_to_offset(funcs_rva).ok_or("AddressOfFunctions")?;
        let mut named: Vec<(u16, String)> = Vec::with_capacity(n_names);
        if n_names > 0 {
            let names_off = self.rva_to_offset(names_rva).ok_or("AddressOfNames")?;
            let ords_off = self.rva_to_offset(ords_rva).ok_or("AddressOfNameOrdinals")?;
            for i in 0..n_names {
                let ord = read_u16(self.bytes, ords_off + i * 2)?;
                let name_rva = read_u32(self.bytes, names_off + i * 4)?;
                let n_off = self.rva_to_offset(name_rva).ok_or("name entry RVA")?;
                named.push((ord, read_cstring(self.bytes, n_off)));
            }
        }
        let mut out = Vec::with_capacity(n_funcs);
        for i in 0..n_funcs {
            let fn_rva = read_u32(self.bytes, funcs_off + i * 4)?;
            if fn_rva == 0 { continue; }
            let name = named.iter().find(|(o, _)| *o as usize == i)
                .map(|(_, n)| n.clone()).unwrap_or_default();
            out.push(Export { ordinal: base_ordinal + i as u32, rva: fn_rva, name });
        }
